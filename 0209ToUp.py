...
class easyExcel:
    ...
    def __init__(self, filename=None):  # 打开文件或者新建文件（如果不存在的话）
        ...
        if filename:
            ...
            self.xlBook = self.xlApp.Workbooks.Open(filename, ReadOnly=False)
        else:
...
def post(PojName, u_token,parent_node_token):
    # win32api.MessageBox(0, "postfun", "node_token", win32con.MB_OK)
